...
class environment(models.Model):
    ...
    _name = 'it_infrastructure.environment'
    ...
    environment_version_id = fields.Many2one(
        'it_infrastructure.environment_version',
        string='Version',
        required=True
    )
    ...
    environment_repository_ids = fields.One2many(
        'it_infrastructure.environment_repository',
        'environment_id',
        string='Repositories'
    )
    ...
    _track = {
        'state': {
            'it_infrastructure.environment_draft':
            lambda self, cr, uid, obj, ctx=None: obj['state'] == 'draft',
            'it_infrastructure.environment_active':
            lambda self, cr, uid, obj, ctx=None: obj['state'] == 'active',
            'it_infrastructure.environment_cancel':
    ...
    @api.one
    @api.returns('it_infrastructure.environment_repository')
    ...
    def action_wkf_set_draft(self, cr, uid, ids, *args):
        ...
        for obj_id in ids:
            wf_service.trg_delete(
                uid, 'it_infrastructure.environment', obj_id, cr)
