...
class infrastructure_restore_database_wizard(osv.osv_memory):
    _name = "it_infrastructure.restore_database.wizard"
    _description = "IT Infrastructure Restore Database Wizard"

    def _get_database_backup(self):
        dump_id = self.env.context.get('active_ids', False)
        if dump_id:
            d = self.env['it_infrastructure.database.backup'].browse(dump_id[0])
            return d
        return False
    ...
    def restore_database(self):
        ...
        dumps = self.env['it_infrastructure.database.backup'].search(
            [('id', 'in', active_ids)])
        for dump in dumps:
